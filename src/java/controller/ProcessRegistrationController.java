...
public class ProcessRegistrationController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("loggedIn".equals(mode)) {
            ...
            if (isUserAdded) {
                userToRegister = userDAO.getUserByEmail(email); // Lấy lại user vừa tạo để có ID
        ...
        // KIỂM TRA ĐĂNG KÝ TRÙNG LẶP (sau khi đã xác định được userID)
        if (userIdToRegister > 0 && regDAO.hasExistingRegistration(userIdToRegister, courseId)) {
            request.setAttribute("errorMessage", "You have already registered for this course.");
            forwardToRegistrationPage(request, response, courseId, userIdToRegister, null, null, null, null);
            return; 
        }

        // TẠO VÀ LƯU ĐƠN ĐĂNG KÝ
        ...
        // Lấy lại đối tượng User đầy đủ để tạo Registration mới
        if (userToRegister == null && userIdToRegister > 0) {
            userToRegister = userDAO.getUserByID(userIdToRegister);
        }
        CourseDAO courseDAO = new CourseDAO();
        Course course = courseDAO.getCourseByIdd(courseId);

        Registration reg = new Registration(0, userToRegister, null, course, selectedPackage, totalCost, "Submitted", new java.util.Date(), null, null, null);
        
        boolean isSaved = regDAO.save(reg);

        if (isSaved) {
            // Đặt thông báo thành công và gửi mail nếu là user mới
             if (!"loggedIn".equals(mode) && "123".equals(userToRegister.getPassword())) {
                ...
                if(emailSent){
                    session.setAttribute("successMessage", "Registration successful! A new account has been created and credentials have been sent to your email.");
                } else {
                    session.setAttribute("successMessage", "Registration successful! New account created, but failed to send password email.");
                }
            } else {
                 session.setAttribute("successMessage", "Registration successful! The course has been added to your account.");
            }
            response.sendRedirect("register-success.jsp");
        } else {
            request.setAttribute("errorMessage", "We could not process your registration at this time. Please try again later.");
            forwardToRegistrationPage(request, response, courseId, userIdToRegister, null, null, null, null);
        }
    }
