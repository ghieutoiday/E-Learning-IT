...
public class UserController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("forget".equals(action)) {
            handleForgetPassword(request, response, session);
            return;
        }
    ...
    private void handleForgetPassword(HttpServletRequest request, HttpServletResponse response, HttpSession session)
            throws ServletException, IOException {
        ...
        LocalDateTime expiry = expireDateTime();

        TokenForgetPassword tokenModel = new model.TokenForgetPassword();
        tokenModel.setToken(token);
        tokenModel.setExpiryTime(expiry);
        tokenModel.setIsUsed(false);
        tokenModel.setUserID(user);

        new TokenDAO().saveToken(tokenModel);

        // Gửi email chứa link reset
        //String link = request.getRequestURL().toString().replace("userController", "reset-password.jsp") + "?token=" + token;
        String link = "http://localhost:9999/E-Learning-IT/usercontroller?action=changePassword&token=" + token;

        boolean success = EmailServiceForPassword.sendForgetPasswordEmail(user.getEmail(), user.getFullName(), link);
        if (success) {
            session.setAttribute("successMessage", "Please check your email.");
            response.sendRedirect("home");
            return;
        } else {
            request.setAttribute("errorMessage", "Can not send email, please check your network.");
            request.getRequestDispatcher("forget-password.jsp").forward(request, response);
        }
